...
- *Geometry Package*
 - Add digital nD Voronoi Covariance Measure support, as well as
  ...
  of feature detection with VCM. (Jacques-Olivier Lachaud, #803)
...
- *Math Package*
    ...
    - Simple Linear Regression tool added (backport from imagene). (David Coeurjolly, #794)

- *For developpers*
     - Google Benchmark can be enabled to allow micro-benchmarking in
         some DGtal unit tests (https://github.com/google/benchmark) (David Coeurjolly, #790)
...
- *Geometry Package*
 - Fix and add concept of CSurfelLocalEstimator and related ground
  ...
  (TrueDigitalSurfaceLocalEstimator). (Jacques-Olivier Lachaud, #803)
 - Add Integral Invariant estimators so that they meet the concept
  ...
  volume and coariance matrix. (Jeremy Levallois, Jacques-Olivier Lachaud, #803)
 - Random-access iterators added in ArithmeticalDSL. (Tristan Roussillon, #801)
...
- *IO Package*
  ...
  - New draw() method for 3D display models (Viewer3D and Board3D)
    to display surfels with prescribed normal vectors (David Coeurjolly, #802).
  - When exporting an 3D visualization to OBJ, a new option will
    rescale the geometry to fit in [-1/2,1/2]^3. (David Coeurjolly, #820)
...
- *Geometry Package*
  - Bug fix in PowerMap construction. (David Coeurjolly, #814)
