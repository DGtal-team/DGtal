        ...
        int argc, char** argv)
{
    ...
    for(auto v : vertices)
    {
        ...
        function_out << w_s[1] << " " << w_s[2] << " " << laplacian_result(i) << " " << real_laplacian_value << " " << input_function(p_i) << std::endl;

        //trace.info() << laplacian_result(i) << " " << laplacian_result(i) / (options.h * options.h) << " " << laplacian_result(i) * options.h << " " << laplacian_result(i) * options.h * options.h << " " << real_laplacian_value << std::endl;
        //trace.info() << laplacian_result(i) << " " << real_laplacian_value << std::endl;
    ...
    trace.info() << "Mean Error / Max Error : " << error.array().abs().mean() << " / " << error.array().abs().maxCoeff(&max_index) << std::endl;
    error_out << options.h << " " << error.array().abs().mean() << " " << error.array().abs().maxCoeff() << std::endl;
...
int main(int argc, char **argv)
{
    ...
    //while(options.h > 0.001)
    {
        std::function<double(const RealPoint&)> input_function;
        std::function<double(const RealPoint&)> result_function;
        if(options.function == 1)
        {
            input_function = cos_function;
            result_function = cos_result;
        }
        else if(options.function == 0)
        {
            input_function = xx_function;
            result_function = xx_result;
        }
        else
        {
            input_function = exp_function;
            result_function = exp_result;
        }
