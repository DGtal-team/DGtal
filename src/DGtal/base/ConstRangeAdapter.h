...
namespace DGtal 
{
   ...
   *
   * NB: A default functor that does nothing is used if no functor type is specified.
   *
   * @tparam TReturnType 
   *
   ...
   */
  template <typename TIterator, typename TFunctor = DefaultFunctor, 
	    typename TReturnType = typename IteratorCirculatorTraits<TIterator>::Value >
  class ConstRangeAdapter
  {
