...
namespace DGtal
{
   ...
   * Smart pointer on \c T based on reference counts (when c > 0) or a
   * simple pointer on \c T. This is useful when instantiating from an
   * Alias<T> object, letting the user specify if it uses smart
   * pointers or simply pointers.
   *
   * @tparam T any type.
   * @see CountedPtr
   */
